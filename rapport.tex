...
På samma sätt som subtraktionsnoden skiver om ekvationen \(a-b\Rightarrow a+((-1) \cdot b)\) så skriver divisionsnoden om ekvationen så att förenklingen istället kan hanteras av multiplikationsnoden. \(a/b\Rightarrow a \cdot (b^{-1})\)
\begin{figure}[h!]
  \centering
...
Strukturen av projektet baserades till viss del på ett examensarbete vid namn Constructing a Computer Algebra System Capable of Generating Pedagogical Step-by-Step Solutions skriven av Dimitrij Lioubartsev (KTH, 2016). Syftet med detta examensarbete är att modifiera ett existerande bibliotek för symbolhantering, SymPy, så att det kan generera stegen som krävs för att kunna lösa en ekvation. Detta var givetvis ej syftet med vårt gymnasiearbete, men examensarbetet gick in i detalj hur en symbolhanterare kan fungera (kapitel 3). Denna text användes för källa då den kändes relevant till vårt arbete. Så som vi kunde använda den för inspiration för hur arbetet skulle läggas upp användes den också som exempel på hur själva rapporten kunde se ut.
...
Programmet strävar efter att förenkla ekvationen så den blir omskriven på polynom form, vilket kan vara fördelaktigt i vissa situationer, dock inte alla.
\begin{savenotes}
\begin{figure}[h!]
  \centering
  \begin{tabular}{l|l}
